...
if (forcetk.Client === undefined) {
     ...
     *
     * Modified by Alessio Valentini on the 10-10-2012 including sync implementation rather than async
     */
    forcetk.Client.prototype.refreshAccessToken = function(callback, error) {
        ...
        $j.ajax({
            ...
            error: error,
            dataType: "json",
            async: false,       // *** make sync call: the app will wait for the token before move on
            timeout: 4000,      // *** timeout because if the token is not coming back after a while the app is not stuck waiting for it
                                // *** IMPROVE: add a complete function in order to understand if the timeout was triggered or not (print here result)
